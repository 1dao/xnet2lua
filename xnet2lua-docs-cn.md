...
- `template_literals = true`（JS/TS）：`` `a ${b} c` `` 读成一个 `str` token，`${ }` 里可以嵌套字符串和模板字符串。
- `regex_literals = true`（JS/TS）：`/` 出现在操作数可以开始的位置时按正则 `/re/flags` 读成 `str`；前一个 token 是标识符、数字、字符串、`)` `]` `}` 或 `this`/`super`/`true`/`false`/`null` 时仍是除号，但闭合 `if`/`while`/`for`/`with` 条件的 `)` 之后可以开始正则（`if (x) /}/.test(s)`）；行尾前没闭合则回退为除号。`${ }` 内的 `//`、`/* */` 注释会被跳过。
- `lifetimes = true`（Rust）：`'a` 生命周期读成 `id`，`'a'` / `'\n'` 仍是字符字面量。
- `raw_strings = true`（Rust）：`r"..."`、`r#"..."#`、`br##"..."##` 按 `#` 层数读成一个 `str`。
- `nested_comments = true`（Rust）：块注释可以嵌套，`/* a /* b */ c */` 整体是一条注释。
- `indent = true` 时，行尾的 `\` 续行把下一行并入当前逻辑行（不产生 `nl`，也不改变缩进）。
