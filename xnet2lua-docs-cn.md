...
- **HTTP 路由跑在 per-request coroutine** 内。路由 handler 可以同步式写法直接调 `xnats.rpc(...)`，yield 等回复，return response。worker 用 per-conn 队列保证 HTTP/1.1 pipelining 顺序。
...
`xhttp.lua` / `xnats.lua` / `xnats_worker.lua` / `xadmin_worker.lua` / `xrouter.lua` / `xhttp_router.lua` 全部按这个模式写——可参考实现。
