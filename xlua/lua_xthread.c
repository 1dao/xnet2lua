...
/* Embedded logging shim (formerly xlua/lua_xlog.lua). Loaded by luaopen_xthread
** so the executable is self-contained: shipping the exe no longer requires a
** companion lua_xlog.lua on disk. Receives the xthread module table as ... and
** augments it with the log_* helpers, level setters, and print/io.stderr
** redirection. Behaviour is identical to the original on-disk script. */
static const char* XTHREAD_LUA_XLOG =
"local xthread = ...\n"
...
"local LOG_VERBOSE, LOG_DEBUG, LOG_INFO = 2, 3, 4\n"
"local LOG_SYSTEM, LOG_WARN, LOG_ERROR, LOG_FATAL = 5, 6, 7, 8\n"
"local _unpack = table.unpack or unpack\n"
...
"end\n"
"local function _join(head, args, start)\n"
"    if not args or start > args.n then return head end\n"
"    local parts = { head }\n"
"    for i = start, args.n do parts[#parts + 1] = _str(args[i]) end\n"
"    return table.concat(parts, '\\t')\n"
"end\n"
"local function _format_arg_count(fmt)\n"
"    local i, count, has_percent = 1, 0, false\n"
"    while true do\n"
...
"        if ok then return _join(s, args, fmt_n + 1) end\n"
"    elseif fmt_n == 0 and has_percent then\n"
"        local ok, s = pcall(string.format, fmt)\n"
"        if ok then return _join(s, args, 1) end\n"
"    end\n"
"    return _join(_str(fmt), args, 1)\n"
"end\n"
...
"end\n"
"local LEVELS = {\n"
"    { 'log_verbose', LOG_VERBOSE, c_log_verbose },\n"
"    { 'log_debug',   LOG_DEBUG,   c_log_debug   },\n"
"    { 'log_info',    LOG_INFO,    c_log_info    },\n"
"    { 'log_system',  LOG_SYSTEM,  c_log_system  },\n"
"    { 'log_warn',    LOG_WARN,    c_log_warn    },\n"
"    { 'log_error',   LOG_ERROR,   c_log_error   },\n"
"    { 'log_fatal',   LOG_FATAL,   c_log_fatal   },\n"
"}\n"
"for i = 1, #LEVELS do\n"
"    local name, level, emit = LEVELS[i][1], LEVELS[i][2], LEVELS[i][3]\n"
"    xthread[name] = function(...)\n"
"        if not c_log_enabled(level) then return true end\n"
"        return emit(_message(...))\n"
"    end\n"
"end\n"
...
"                    local n = select('#', ...)\n"
"                    local parts = {}\n"
"                    for i = 1, n do parts[i] = _str((select(i, ...))) end\n"
"                    c_log_error(table.concat(parts))\n"
"                end\n"
"                return stderr_proxy\n"
"            end,\n"
...
static int lua_xthread_load_xlog(lua_State* L) {
    return luaL_loadstring(L, XTHREAD_LUA_XLOG);
}
