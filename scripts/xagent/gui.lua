...
local function __init()
    ...
    -- Local viz static server: pages generated by the aetherviz skill are written
    -- here and opened in the browser. Best-effort — never block GUI boot.
    do
        local vp, ve = viz.ensure_server()
        if vp then
            io.stderr:write(string.format('[viz] serving %s on http://%s:%d/\n',
                viz.paths.root(), viz.paths.host(), vp))
        elseif ve then
            io.stderr:write('[viz] static server not started: ' .. tostring(ve) .. '\n')
        end
    end
