...
spec.describe('xhttp_client.parse_proxy', function()
    spec.it('parses socks5 host:port', function()
        local p = httpc.parse_proxy('socks5://127.0.0.1:1080')
        spec.equal(p.type, 'socks5')
        spec.equal(p.host, '127.0.0.1')
        spec.equal(p.port, 1080)
    end)

    spec.it('defaults the socks5 port to 1080', function()
        local p = httpc.parse_proxy('socks5h://host')
        spec.equal(p.type, 'socks5h')
        spec.equal(p.host, 'host')
        spec.equal(p.port, 1080)
    end)

    spec.it('defaults the http proxy port to 8080', function()
        local p = httpc.parse_proxy('http://proxy.local')
        spec.equal(p.type, 'http')
        spec.equal(p.port, 8080)
    end)

    spec.it('extracts user:pass credentials', function()
        local p = httpc.parse_proxy('http://u:p@h:3128')
        spec.equal(p.host, 'h')
        spec.equal(p.port, 3128)
        spec.equal(p.user, 'u')
        spec.equal(p.pass, 'p')
    end)

    spec.it('parses a bracketed IPv6 proxy literal', function()
        local p = httpc.parse_proxy('socks5://[::1]:1080')
        spec.equal(p.host, '::1')
        spec.equal(p.port, 1080)
    end)

    spec.it('rejects a scheme-less proxy url', function()
        local p, err = httpc.parse_proxy('127.0.0.1:1080')
        spec.nil_value(p)
        spec.truthy(err)
    end)

    spec.it('rejects an unsupported proxy scheme', function()
