...
- `template_literals = true` (JS/TS): `` `a ${b} c` `` is one `str` token; `${ }` parts may nest strings and further templates.
- `regex_literals = true` (JS/TS): a `/` where an operand may start is read as a `/re/flags` `str`; after an identifier, number, string, `)` `]` `}` or `this`/`super`/`true`/`false`/`null` it stays a division, except after a `)` that closes an `if`/`while`/`for`/`with` condition (`if (x) /}/.test(s)`); a regex left open at the end of the line falls back to a division. `//` and `/* */` comments inside `${ }` are skipped.
- `lifetimes = true` (Rust): `'a` lifetimes are `id` tokens while `'a'` / `'\n'` stay char literals.
- `raw_strings = true` (Rust): `r"..."`, `r#"..."#`, `br##"..."##` are one `str`, matched by the number of `#`.
- `nested_comments = true` (Rust): block comments nest, so `/* a /* b */ c */` is one comment.
- With `indent = true`, a trailing `\` joins the next line to the current logical line (no `nl`, no indentation change).
